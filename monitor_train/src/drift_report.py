...
def detect_drift_keys(model_config, request_file, key):
    drift_keys = []
    model_config_path = (AppPath.MODEL_CONFIG_DIR / model_config).as_posix()
    with open(model_config_path, "r") as f:
        config = yaml.safe_load(f)
    prob_config = create_prob_config(config["phase_id"], config["prob_id"])
    ...
    for key in saved_request:
        ...
        raw_df=saved_request[key][prob_config.feature_cols],
        categorical_cols=prob_config.categorical_cols,
        category_index=category_index,
            )
        if ks_drift(reference.to_numpy(), current.to_numpy())[0]==1:
            print(key)
